...
<body>
	...
	<p id="lessonCount">등록된 수업 <%=listCount %>개</p>
	 ...
	 <div class="pagingArea" align="center">
			<button onclick="location.href='<%= request.getContextPath()%>/selectDesign.le?currentPage=1'"><<</button>	
			<% if(currentPage <= 1) { %>
				<button disabled><</button>	
			<% }else { %>
				<button onclick="location.href='<%= request.getContextPath()%>/selectDesign.le?currentPage=<%=currentPage - 1%>'"><</button>	
			<% } %>
			...
			%>	
					<button onclick="location.href='<%= request.getContextPath()%>/selectDesign.le?currentPage=<%=p%>'"><%= p %></button>
			<%	}
			  }	
			%>		
			...
			<% }else { %>
				<button onclick="location.href='<%= request.getContextPath()%>/selectDesign.le?currentPage=<%=currentPage + 1%>'">></button>
			<% } %>
			<button onclick="location.href='<%= request.getContextPath()%>/selectDesign.le?currentPage=<%=maxPage%>'">>></button>	
		</div>
	<script>
		$(function() {
			$("#cDesign").css({'color':'darkolivegreen'});
			$("#design").show();	
